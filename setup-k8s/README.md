...
## 1. Build the Financial Tracker image

```bash
docker build -t financial-tracker:k8s .
```
...
```bash
kind load docker-image financial-tracker:k8s --name tws-cluster
```
...
kubectl apply -f k8s/financial-tracker-deployment.yml
...
```bash
kubectl get all -n financial-tracker
kubectl get hpa -n financial-tracker            # wait ~60s for metrics to populate
kubectl top pods -n financial-tracker
```
...
Generate CPU load on financial-tracker to trigger scale-up:

```bash
kubectl run load-test --image=busybox:1.36 -n financial-tracker -- sh -c "while true; do wget -q -O- http://financial-tracker-service:8080/health >/dev/null 2>&1; done"
```

Watch replicas scale from 2 → 4 (takes ~1-2 min for metrics to kick in):

```bash
kubectl get hpa -n financial-tracker -w
```

Clean up when done:

```bash
kubectl delete pod load-test -n financial-tracker
```
