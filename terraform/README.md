# AI Financial Tracker - End-to-End GitOps on EKS

Complete guide to deploy AI Financial Tracker on AWS EKS using Terraform, ArgoCD, Gateway API, and Prometheus monitoring.
...
| **EKS Cluster** | `Financial Tracker-eks` running Kubernetes **1.35** |
| **Node Group** | `Financial Tracker-ng` — 3x `t3.medium` instances on AL2023 |
...
```bash
aws eks update-kubeconfig --name Financial Tracker-eks --region us-west-2
kubectl get nodes
```
...
- `Financial Tracker.trainwithshubham.com` → `<NLB hostname from Gateway>`
...
kubectl get certificate -n Financial Tracker
kubectl describe certificate -n Financial Tracker
```
...
2. Click **New App** → fill in:
   ...
   - **Project:** `default`
   - **Repo URL:** `https://github.com/TrainWithShubham/AI-Financial Tracker-DevOps.git`
...
|----------|-------------|
| `namespace.yml` | Creates `Financial Tracker` namespace |
...
| `Financial Tracker-deployment.yml` | Financial Tracker (2 replicas, HPA enabled) |
| `service.yml` | ClusterIP services for all components |
| `gateway.yml` | Gateway API → external access via NLB |
| `hpa.yml` | Auto-scale Financial Tracker 2-4 replicas |

## 8. Verify Deployment

```bash
# Pods
kubectl get pods -n Financial Tracker

# Services
kubectl get svc -n Financial Tracker

# Gateway + NLB
kubectl get gateway -n Financial Tracker

# Get the app URL
export APP_URL=$(kubectl get svc -n envoy-gateway-system \
  -l gateway.envoyproxy.io/owning-gateway-name=Financial Tracker-gateway \
  -o jsonpath='{.items[0].status.loadBalancer.ingress[0].hostname}')
echo "Financial Tracker: http://$APP_URL"
```
...
2. **GitHub Actions** (`.github/workflows/gitops-ci.yml`):
   ...
   - Builds Docker image
   - Pushes `trainwithshubham/ai-Financial Tracker-eks:<sha>` to DockerHub
   - Updates `k8s/Financial Tracker-deployment.yml` with new image tag
...
kubectl describe application Financial Tracker -n argocd

# Pod logs
kubectl logs -l app=Financial Tracker -n Financial Tracker --tail=50

# EBS PVC status (should be Bound)
kubectl get pvc -n Financial Tracker

# Gateway status
kubectl get gateway,httproute -n Financial Tracker
```
